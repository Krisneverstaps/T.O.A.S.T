...
df = load_snana_format(hd_path).merge(load_snana_format(meta_path)[['CID', 'HOST_COLOR', 'mB', 'x1', 'c', "x0", "biasCor_mu", "biasCorErr_mu"]], on='CID')
...
bin_centers, bin_means, bin_errs = binned_weighted_mean(
    df['HOST_COLOR'].values, df['hubble_residual'].values, df['MUERR'].values, df['biasCorErr_mu'].values,  bins=6
)
...
plt.savefig(out_dir / "DES_HRvsColour.png", dpi=150)
plt.show()
