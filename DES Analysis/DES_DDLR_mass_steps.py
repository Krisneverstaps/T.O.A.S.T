...
df = df[df['PROBIA_BEAMS'] > 0.95].dropna(subset=['zHD', 'mB', 'x1', 'c', 'HOST_LOGMASS', 'MUERR', 'HOST_DDLR', "x0"])

# PHYSICS
df = calculate_physics(df)

# SPLIT DATA BASED ON DDLR
df = df[df["HOST_DDLR"] < 4]   # Restricting to SNe that are within the region of the galaxy
close_df = df[df["HOST_DDLR"] < 1] # 1 represents the SNe is on the edge of the galaxy
far_df = df[df["HOST_DDLR"] >= 1]
...
def compute_mass_step(group_df, title_suffix):
    ...
    plt.savefig(out_dir / f"DES_mass_step_{title_suffix.replace(' ', '_')}.png", dpi=150)
    plt.show()
