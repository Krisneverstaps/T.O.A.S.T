...
df = df.dropna(subset=["zHD", "mB", "x1", "c", "HOST_LOGMASS", "MUERR", "x0", "concentration"])

print(f"Final dataset contains {len(df)} supernovae.")
print(df[["CID", "HOST_LOGMASS", "concentration"]].head())
...
def compute_mass_step(group_df, title_suffix):
    ...
    plt.savefig(out_dir / f"Euc_Morph_conc(nobias).png", dpi=150)
    plt.show()
