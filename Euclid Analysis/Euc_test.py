...
m_col = "logmass" if "logmass" in df_euclid.columns else "SFH_AGE"
df_euclid["SFH_AGE"] = pd.to_numeric(df_euclid[m_col], errors="coerce")


#Load morphology and remove the commas from numbers like 1,000,000
df_morph = pd.read_csv(euclid_morph_path, thousands=',')

print(df_morph["concentration"])
...
df_morph["concentration"] = pd.to_numeric(df_morph["concentration"], errors="coerce")

# ADD concentration TO EUCLID DATA 
df_euclid = df_euclid[["DES_ID_x", "SFH_AGE"]].merge(
    df_morph[["DES_ID_x", "concentration"]], 
    on="DES_ID_x", 
    how="inner"
)
...
df = df_euclid[["DES_ID_x", "SFH_AGE", "concentration"]].merge(
...
df = df.dropna(subset=["zHD", "mB", "x1", "c", "SFH_AGE", "MUERR", "x0", "concentration"])

print(f"Final dataset contains {len(df)} supernovae.")
print(df[["CID", "SFH_AGE", "concentration"]].head())

# PHYSICS
df = calculate_physics(df)


LTG_df = df[df["concentration"] <=2.8 ]
ETG_df = df[df["concentration"] > 2.8 ]


def compute_mass_step(group_df, title_suffix):
    """Compute and plot Hubble residual vs host mass for a given group."""
    low_df = group_df[group_df['SFH_AGE'] < group_df['SFH_AGE'].median()]
    high_df = group_df[group_df['SFH_AGE'] >= group_df['SFH_AGE'].median()]
    ...
    bin_centers, bin_means, bin_errs = binned_weighted_mean(
        group_df['SFH_AGE'].values, group_df['hubble_residual'].values, group_df['MUERR'].values, group_df["biasCorErr_mu"].values, bins=6
    )
    ...
    print(f"SFH_AGE range: "
          f"{group_df['SFH_AGE'].min():.2f} -- "
          f"{group_df['SFH_AGE'].max():.2f}")
    ...
    ax.scatter(group_df['SFH_AGE'], group_df['hubble_residual'], alpha=0.2, color='gray', s=15, label='SNe Ia')
    ...
    ax.hlines(w_mean_low, group_df['SFH_AGE'].min(), 10, colors='red', lw=2, label=f'Low SFH_AGE: {w_mean_low:.3f} ± {w_err_low:.3f}')
    ax.hlines(w_mean_high, 10, group_df['SFH_AGE'].max(), colors='orange', lw=2, label=f'High SFH_AGE: {w_mean_high:.3f} ± {w_err_high:.3f}')
    ...
    ax.set_title(f'Hubble Residual vs SFH_AGE ({title_suffix}, SFH_AGE step: {mass_step:.3f} mag)')
    ...
    plt.savefig(out_dir / f"hubble_residual_vs_SFH_AGE_{title_suffix.replace(' ', '_')}.png", dpi=150)
    plt.show()
