...
df_merged = df_merged.dropna(subset=["ANGSEP", "HOST_ANGSEP"])


plt.figure(figsize=(8, 8))


max_val = max(df_merged["ANGSEP"].max(), df_merged["HOST_ANGSEP"].max())
plt.plot([0, max_val], [0, max_val], color='red', linestyle='--', label='1:1 Agreement')

# Plot the data
plt.scatter(
    df_merged["HOST_ANGSEP"], 
    df_merged["ANGSEP"], 
    alpha=0.5, 
    color='blue', 
    edgecolor='k',
...
plt.xlabel("DES HOST_ANGSEP (arcsec)")
plt.ylabel("Euclid Calculated ANGSEP (arcsec)")
plt.legend()
plt.grid(True, alpha=0.3)

# STATISTICS
correlation = df_merged["HOST_ANGSEP"].corr(df_merged["ANGSEP"])
