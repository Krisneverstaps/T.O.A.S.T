...
def calculate_physics2(df, alpha=ALPHA, beta=BETA, m0=M0_AVG, omega_m=OMEGA_M):
    """Calculates expectations, residuals, and corrections (DES 5-year parameters)."""
    cosmo = FlatLambdaCDM(H0=70, Om0=omega_m)
...
def binned_weighted_mean2(x, y, observational_errors,  bins=6, sigma_int=0.11):
    ...
    for i in range(bins):
        ...
        if i == bins - 1:
            mask = (x >= edges[i]) & (x <= edges[i + 1])
        if np.sum(mask) > 0:
            means[i], errs[i] = get_weighted_stats2(y[mask], observational_errors[mask], sigma_int=sigma_int)
    return centers, means, errs
