...
df_euclid["METALLICITY"] = pd.to_numeric(df_euclid["phz_pp_mode_stellarmetallicity"], errors="coerce")
...
# PRINT SUMMARY
print("=" * 60)
print("Euclid: Hubble Residual vs METALLICITY (phz_pp_mode_METALLICITY)")
print("=" * 60)
print(f"phz_pp_mode_METALLICITY range: {df['METALLICITY'].min():.2f} -- {df['METALLICITY'].max():.2f}")
print(f"Redshift range: {df['zHD'].min():.4f} -- {df['zHD'].max():.4f}")
print(f"Total SNe: {len(df)}")
print(f"Split at median METALLICITY: {METALLICITY_split:.2f}")
print()
print(f"Low METALLICITY (METALLICITY < {METALLICITY_split:.2f}):  {len(low_df)} SNe")
print(f"High METALLICITY (METALLICITY >= {METALLICITY_split:.2f}): {len(high_df)} SNe")
print()
print(f"Mean residual (low METALLICITY):  {w_mean_low:+.4f} ± {w_err_low:.4f} mag")
print(f"Mean residual (high METALLICITY): {w_mean_high:+.4f} ± {w_err_high:.4f} mag")
print(f"METALLICITY step (high - low):    {METALLICITY_step:+.4f} mag")
print()
...
print(f"  {'Bin center (METALLICITY)':<22} {'Weighted mean (mag)':<22} {'Uncertainty (mag)':<18}")
print("-" * 65)
...
ax.scatter(df["METALLICITY"], df["hubble_residual"], alpha=0.2, color="gray", s=15, label="Euclid SNe Ia")
...
x_min, x_max = df["METALLICITY"].min(), df["METALLICITY"].max()
ax.hlines(w_mean_low, x_min, METALLICITY_split, colors="red", lw=2, label=f"Low METALLICITY: {w_mean_low:.3f} ± {w_err_low:.3f}")
ax.hlines(w_mean_high, METALLICITY_split, x_max, colors="orange", lw=2, label=f"High METALLICITY: {w_mean_high:.3f} ± {w_err_high:.3f}")
ax.fill_between([x_min, METALLICITY_split], w_mean_low - w_err_low, w_mean_low + w_err_low, color="red", alpha=0.2)
ax.fill_between([METALLICITY_split, x_max], w_mean_high - w_err_high, w_mean_high + w_err_high, color="orange", alpha=0.2)
ax.axvline(METALLICITY_split, color="black", linestyle="--")
ax.set_title(f"Euclid: Hubble Residual vs METALLICITY (phz_pp_mode_METALLICITY, step: {METALLICITY_step:.3f} mag)")
ax.set_xlabel("phz_pp_mode_METALLICITY")
...
plt.savefig(out_dir / "euclid_hubble_residual_vs_METALLICITY_step.png", dpi=150)
plt.show()
